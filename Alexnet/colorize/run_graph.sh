...
if [ ! -f ../../pretrain/alexnet_colorize.caffemodel.h5 ]; then
    echo pre-trained file not found. Please execute download_pretrain.sh.
    exit
fi

$CAFFE train --solver=solver_graph.prototxt --weights ../../pretrain/alexnet_colorize.caffemodel.h5 2>&1 |tee log/graph.log
