...
void NormalizeLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int i=0; i<n; ++i) {
    ...
    if (abs(pow(normsqr, 0.5) - 0) < 1e-30)
        caffe_cpu_scale<Dtype>(d, Dtype(1), bottom_data+i*d, top_data+i*d);
    else
...
void NormalizeLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  for (int i=0; i<n; ++i) {
    ...
    if (abs(Dtype(pow(a, 0.5)) - 0) < 1e-30)
        caffe_cpu_scale(d, Dtype(1), bottom_diff+i*d, bottom_diff+i*d);
    else
